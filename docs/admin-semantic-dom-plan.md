...
- `assets/common/tokens.css`: 사이트 공통 디자인 토큰
- `assets/common/ui-kit.css`, `assets/common/utilities.css`: 공통 UI kit 원형과 재사용 유틸리티

## 명명 원칙

| 계층 | 사용 class | 책임 |
| --- | --- | --- |
| 공통 UI | `card`, `btn`, `table`, `table-wrapper`, `form-*`, `tab-*` | UI kit 원형 |
...
UI kit의 `table-wrapper > table.table`을 유지하되 관리자 의미 class를 추가한다.
...
UI kit의 탭 구조를 따른다. 페이지 이동용 링크 묶음은 탭이 아니라 페이지 액션/로컬 내비게이션으로 분리한다.
...
상태 표시는 UI kit의 badge 계열을 관리자 의미 class와 결합한다.
...
- `assets/admin-ui.css`
- `assets/common/tokens.css`
- `assets/common/ui-kit.css`
- `assets/common/utilities.css`
- `modules/admin/assets/admin.css`

작업:

- 공통 토큰, UI kit 원형, 공통 유틸은 `assets/common/` 아래 split 파일에 둔다.
- `assets/common.css`는 정적 UI-KIT과 호환을 위한 manifest로 유지한다.
- 관리자 런타임은 `assets/common/tokens.css`, `assets/common/ui-kit.css`, `assets/common/utilities.css`, `assets/admin-ui.css`, `modules/admin/assets/admin.css` 순서로 직접 호출한다.
