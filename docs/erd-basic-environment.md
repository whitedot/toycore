...
MVP에서는 이 테이블 없이 시작할 수 있습니다. 관리자 설정 변경, 모듈 변경, 회원 상태 변경 같은 상태 변경 기능이 들어갈 때 함께 도입합니다.
...
MVP에서는 만들지 않고, 계정 화면이나 프로필 수정 기능이 들어갈 때 도입합니다.
...
MVP에서는 PHP 기본 세션으로 시작하고, 강제 로그아웃이나 장기 로그인 요구가 생기면 도입합니다.
...
MVP에서는 공개 회원가입을 포함하지 않으므로 후속 단계에서 도입합니다.
...
MVP에서는 만들지 않고, 개인정보 요청 관리 화면을 구현하는 단계에서 추가합니다.
