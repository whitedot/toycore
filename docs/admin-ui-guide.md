...
관리자 메뉴 표시 설정 화면은 분류, 모듈 그룹, 항목을 같은 저장 표에서 다루되 대상 칸에서 들여쓰기와 보조 경로로 위계를 드러낸다. 순서 드래그는 같은 범위와 같은 부모 안에서만 가능하게 유지하고, 이동 가능한 위치에만 고스트 라인을 표시한다. 분류나 모듈 그룹을 드래그하면 하위 행도 같은 블록으로 함께 이동하며, 저장 후 다시 펼칠 때도 부모와 자식 행이 분리되지 않아야 한다. 저장/초기화 액션 영역은 긴 메뉴 목록에서도 접근 가능하도록 화면 하단에 sticky로 유지한다.
...
사이트 설정 화면은 사이트 이름, 시간대, locale, 운영 상태, 공개 화면 레이아웃, UI 색상 모드, 관리자 스킨처럼 운영자가 의미를 이해하고 직접 변경할 수 있는 전용 필드만 다룬다. 공개 화면과 관리자 화면 표시 옵션은 같은 `화면` 섹션에서 한 번에 저장한다. 탑바 다크 모드 전환 버튼은 `ui_color_scheme` 값을 라이트/다크로 즉시 저장하는 빠른 전환이며, 시스템 설정 모드는 사이트 설정 화면에서 선택한다. `sr_site_settings`는 내부 저장소로 유지하지만, 임의 key/value를 추가·삭제하는 범용 관리자 화면은 제공하지 않는다.
