...
최초 관리자 계정은 이메일 로그인 또는 아이디 로그인을 선택할 수 있습니다. 아이디 로그인 선택 시에도 이메일은 비밀번호 재설정과 계정 안내를 위해 저장합니다.
...
최초 관리자 로그인 식별자는 설치 화면에서 선택합니다.

```text
이메일 로그인: account_identifier_hash = email_hash
아이디 로그인: account_identifier_hash = login_id_hash, email_hash는 별도 저장
```
