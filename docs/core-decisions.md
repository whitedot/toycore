...
`seo`와 `popup_layer` 모듈은 기본 배포에 함께 포함하고 기본 활성화하지만, 인증과 관리자 진입에 필요한 필수 모듈은 아닙니다. sitemap 같은 SEO 운영 기능은 코어가 아니라 `seo` 모듈 책임으로 두고, 팝업 정책과 대상 규칙은 `popup_layer` 모듈 책임으로 둡니다.
