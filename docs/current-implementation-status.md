...
이 문서는 현재 코드 기준의 구현 범위와 검증 기준을 정리한다.

사용자에게 보이는 기능 목록은 [구현된 기능 리스트](implemented-features.md)를 기준으로 보고, 구현 판단이 갈릴 때는 [핵심 설계 결정](core-decisions.md)을 우선한다. 이 문서는 릴리스 전 현재 코드가 어디까지 반영되었는지 점검하는 운영용 상태표로 유지한다.
