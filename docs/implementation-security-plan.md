...
웹에서 직접 접근 가능한 진입점입니다. 가능한 한 `public/index.php` 하나를 중심으로 요청을 받고, 업로드 파일이나 설정 파일이 웹 루트에서 직접 실행되지 않도록 합니다.

최초 진입점은 코어가 책임집니다. `page` 같은 특정 모듈이 `/` 요청 자체를 항상 소유하지 않습니다. 홈 화면의 콘텐츠 제공자는 사이트 설정으로 결정하고, 설정된 모듈이 활성 상태일 때만 해당 모듈 handler를 include합니다.
...
요청 처리 시 코어는 현재 사이트의 활성 모듈 목록만 읽고, 활성 모듈의 path 매핑만 확인합니다. 현재 요청과 일치하는 handler만 검증 후 include합니다.
...
이 코드는 개념 예시입니다. 실제 구현에서는 `module_key`와 handler 상대 경로를 검증한 뒤, 현재 요청에 필요한 handler만 include합니다. `paths.php`는 전역 라우트 등록을 수행하지 않고 배열만 반환합니다.
...
- 의존성 주입 컨테이너
- 이벤트 버스 기반의 숨은 실행 흐름
- annotation, attribute, reflection 기반 라우트 자동 등록
...
초기 구현에서는 사이트 기본 locale과 회원 locale을 우선 지원하고, URL 기반 다국어 라우팅은 후순위로 둡니다.
...
`public/index.php`는 코어 진입점입니다. 홈 화면은 코어가 직접 고정하지 않고 사이트 설정으로 결정합니다.
...
- 디렉터리 구조 확정
- `public/index.php` 진입점 구성
- `config/config.php` 또는 `config/local.php` 방식 결정
- PDO 기반 DB 연결
- 공통 응답, redirect, escaping 헬퍼 작성
- 사이트 기본 locale과 지원 locale 설정
- `toy_sites`, `toy_modules`, `toy_site_modules`, `toy_site_settings`, `toy_module_settings` 설치 SQL 작성
...
관리자 화면도 절차형 원칙을 따릅니다. `admin` 모듈의 path 매핑은 `paths.php` 배열로 제공하고, 코어가 활성 상태와 권한을 검증한 뒤 handler를 include합니다.
...
- 새 모듈을 추가해 라우트와 화면을 붙일 수 있음
- 모듈 비활성화 시 해당 라우트가 차단됨
...
- 설치 후 기본 관리자 계정과 기본 비밀번호를 강제로 변경
...
| 모듈과 코어 결합 | 기능 교체 어려움 | 모듈은 등록, 라우트, 설정 규칙으로 연결 |
...
- `docs/routing-entry-plan.md`: 진입점과 홈 라우팅 계획
