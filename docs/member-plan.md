...
인증 메일과 재설정 메일의 전송 성공 여부는 토큰 원문 없이 감사 로그 metadata에 기록합니다.
메일 발송 실패는 별도 인증 로그 이벤트로도 남깁니다.
...
로그인 세션 생성 실패도 로그인 실패 제한 기준에 포함합니다.
...
계정 ID 없이 회원 session token hash만 남은 부분 로그인 상태는 해당 token을 폐기하고 비로그인으로 처리합니다.
