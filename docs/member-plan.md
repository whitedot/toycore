...
| 커머스 활동 | 주문, 배송지, 장바구니, 쿠폰, 적립금, 구매 등급 | `shop`, `order` 등 | 커머스 모듈이 정책과 테이블 소유 |
...
-> 인증 메일 발송 요청
-> 인증 로그 기록
-> 가입 완료 또는 인증 안내 화면 출력
```

초기 구현에서는 이메일 발송 helper가 준비되지 않을 수 있습니다. 이 경우 인증 토큰 생성 구조와 상태값을 먼저 마련하고, 실제 발송은 후속 구현으로 분리할 수 있습니다.
