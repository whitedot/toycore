...
후순위: hreflang, SEO 관리자 UI, 분석 도구 연동
```
...
`seo` 모듈은 기본 `/robots.txt`를 제공합니다. 기본 출력은 관리자, 계정, 로그인, 가입, 비밀번호 재설정 경로를 `Disallow`하고 `/sitemap.xml` 위치를 `Sitemap`으로 안내합니다. 관리자 화면에서 직접 편집하는 기능은 후순위입니다.
...
현재 구현은 `toy_seo_tags()`와 `toy_canonical_url()`을 제공한다. 각 view 또는 모듈 action은 `$seo` 배열에 필요한 값만 명시하고, 코어 helper는 전달된 값을 escape한 뒤 `<head>` 출력 태그로 변환한다. 코어는 콘텐츠 의미를 추론하지 않으며 sitemap, hreflang, SEO 관리자 UI는 후순위로 둔다.
