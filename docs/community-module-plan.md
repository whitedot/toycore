...
| 릴리스 준비 | 진행 중 | `php .tools/bin/check.php`는 커뮤니티 v1 릴리스 기준, enabled 모듈 route 로딩, 패키지 최상위 구조, 계약 파일 구조, 설치 SQL 핵심 구조, sitemap/privacy export 범위, 스타일 경계, 회원 전용 action guard, 계약 파일 선언/존재, 모듈 버전/update SQL 정책을 확인하고, HTTP smoke에 커뮤니티 public/admin 진입점, 비로그인 작성/쪽지/스크랩 guard, 내부 파일 보호 점검이 포함됨. 로그인 세션 성공 흐름은 `.tools/bin/smoke-community-auth.php`로 점검할 수 있음 |
...
- `.tools/bin/smoke-community-auth.php` 인증 HTTP smoke(로그인 후 작성 결과/댓글/스크랩/쪽지 발송·수신 동일 본문 화면 반영, 두 번째 계정으로 게시글 신고, 해당 신고 관리자 처리와 게시글 숨김)
