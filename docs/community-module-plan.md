...
| 릴리스 준비 | 진행 중 | `php .tools/bin/check.php`는 커뮤니티 v1 릴리스 기준, enabled 모듈 route 로딩, 패키지 최상위 구조, 계약 파일 구조, 설치 SQL 핵심 구조, sitemap/privacy export 범위, 스타일 경계, 회원 전용 action guard, 계약 파일 선언/존재, 모듈 버전/update SQL 정책을 확인하고, HTTP smoke에 커뮤니티 public/admin 진입점, 비로그인 작성/쪽지/스크랩 guard, 내부 파일 보호 점검이 포함됨. 로그인 세션 성공 흐름과 쪽지 수신자 해시 링크 노출은 `.tools/bin/smoke-community-auth.php`로 점검할 수 있음 |

현재 남은 우선순위:

1. 실제 로그인 계정과 게시판/수신자/신고자/관리자 데이터가 준비된 환경에서 `.tools/bin/smoke-community-auth.php`를 실행해 작성/댓글/쪽지/신고/스크랩/관리자 처리 성공 흐름을 확인한다.
...
- `.tools/bin/smoke-http.php` 기본 HTTP smoke(public/admin 진입점, 비로그인 작성/쪽지/스크랩 guard, 내부 파일 보호)
- `.tools/bin/smoke-http.php`는 CLI 인자 또는 `TOY_SMOKE_BASE_URL` 환경변수로 base URL 주입 가능
- `.tools/bin/smoke-community-auth.php` 인증 HTTP smoke(로그인 후 작성 결과/댓글/스크랩/쪽지 발송·수신 동일 본문 화면 반영, 쪽지 폼/답장 링크의 숫자 회원 id 비노출과 공개 해시 링크, 두 번째 계정으로 게시글 신고, 해당 신고 관리자 처리와 게시글 숨김)
...
- 설치 전/후 시나리오 확인
- zip 구조 확인은 패키지 최상위 구조 정적 검사로 우선 방어하고, 실제 압축 산출물은 배포 직전에 확인
