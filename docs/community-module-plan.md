...
| 릴리스 준비 | 진행 중 | `php .tools/bin/check.php`는 커뮤니티 v1 릴리스 기준, 계약 파일 선언/존재, 모듈 버전/update SQL 정책을 확인하고, HTTP smoke에 커뮤니티 public/admin 진입점과 내부 파일 보호 점검이 포함됨 |

현재 남은 우선순위:

1. 인증이 필요한 커뮤니티 작성/쪽지/스크랩 흐름은 수동 HTTP smoke로 확인한다.
...
- `.tools/bin/smoke-http.php` 기본 HTTP smoke(public/admin 진입점과 내부 파일 보호)
- 인증이 필요한 흐름은 수동 HTTP smoke
- 설치 전/후 시나리오 확인
- zip 구조 확인
