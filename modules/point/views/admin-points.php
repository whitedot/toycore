...
<p>
    <a href="<?php echo toy_e(toy_url('/admin/points/balances')); ?>">잔액</a>
    |
    <a href="<?php echo toy_e(toy_url('/admin/points/adjust')); ?>">조정</a>
    |
    <a href="<?php echo toy_e(toy_url('/admin/points/transactions')); ?>">거래 내역</a>
</p>

<section>
    <h2>회원 조회</h2>
    <form method="get" action="<?php echo toy_e(toy_url($pointAdminPage === 'transactions' ? '/admin/points/transactions' : ($pointAdminPage === 'adjust' ? '/admin/points/adjust' : '/admin/points/balances'))); ?>">
        <label>회원 공개 해시<br>
...
<?php if ($pointAdminPage === 'adjust') { ?>
    <section>
        <h2>포인트 조정</h2>
        <form method="post" action="<?php echo toy_e(toy_url('/admin/points/adjust' . ($accountIdentifierFilter !== '' ? '?account_identifier=' . rawurlencode($accountIdentifierFilter) : ''))); ?>">
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($balances as $balance) { ?>
                        <tr>
                            <td><a href="<?php echo toy_e(toy_url('/admin/points/transactions?account_identifier=' . rawurlencode((string) $balance['account_public_hash']))); ?>"><?php echo toy_e((string) $balance['account_public_hash']); ?></a></td>
