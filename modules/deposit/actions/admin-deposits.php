...
require_once TOY_ROOT . '/modules/deposit/helpers.php';
...
if (toy_request_method() === 'POST') {
    ...
    $reason = toy_deposit_clean_text(toy_post_string('reason', 255), 255);
    $referenceType = toy_deposit_clean_key(toy_post_string('reference_type', 60), 60);
    $referenceId = toy_deposit_clean_reference_id(toy_post_string('reference_id', 120), 120);
    ...
    if ($errors === []) {
        try {
            $transactionId = toy_deposit_create_transaction($pdo, [
...
if ($accountIdFilter > 0) {
    ...
    if (is_array($row)) {
        $selectedAccount = $row;
        $selectedBalance = toy_deposit_balance($pdo, $accountIdFilter);
    }
}
...
if ($accountIdFilter > 0) {
    $stmt = $pdo->prepare(
        'SELECT t.id, t.account_id, t.amount, t.balance_after, t.transaction_type, t.reason, t.reference_type, t.reference_id, t.created_by_account_id, t.created_at,
         ...
         FROM toy_deposit_transactions t
...
include TOY_ROOT . '/modules/deposit/views/admin-deposits.php';
