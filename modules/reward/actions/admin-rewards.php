...
require_once TOY_ROOT . '/modules/reward/helpers.php';
...
if (toy_request_method() === 'POST') {
    ...
    $reason = toy_reward_clean_text(toy_post_string('reason', 255), 255);
    $referenceType = toy_reward_clean_key(toy_post_string('reference_type', 60), 60);
    $referenceId = toy_reward_clean_reference_id(toy_post_string('reference_id', 120), 120);
    ...
    if (preg_match('/\A-?\d+\z/', $amountInput) !== 1) {
        $errors[] = '적립금 금액은 정수로 입력하세요.';
    }

    $amount = (int) $amountInput;
    if ($amount === 0) {
        $errors[] = '적립금 금액은 0일 수 없습니다.';
    }
    ...
    if ($errors === []) {
        try {
            $transactionId = toy_reward_create_transaction($pdo, [
...
if ($accountIdFilter > 0) {
    ...
    if (is_array($row)) {
        $selectedAccount = $row;
        $selectedBalance = toy_reward_balance($pdo, $accountIdFilter);
    }
}
...
if ($accountIdFilter > 0) {
    $stmt = $pdo->prepare(
        'SELECT t.id, t.account_id, t.amount, t.balance_after, t.transaction_type, t.reason, t.reference_type, t.reference_id, t.created_by_account_id, t.created_at,
         ...
         FROM toy_reward_transactions t
...
include TOY_ROOT . '/modules/reward/views/admin-rewards.php';
